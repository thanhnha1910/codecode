...
const TransportationImages = () => {
  const { id } = useParams(); // Extract transportation ID from URL
  ...
  const apiUrl = `http://localhost:5128/api/images?entityType=transportation&entityID=${id}`;
  console.log("Transportation ID:", id);
  ...
  const handleAddImages = async (e) => {
    ...
    try {
      const response = await axios.post(
        `http://localhost:5128/api/images/multiple?entityType=transportation&entityID=${id}`,
        formData,
        {
          headers: {
  ...
  const handleDeleteImage = async (imageId) => {
    ...
    try {
      ...
      const response = await axios.delete(
        `http://localhost:5128/api/images/${imageId}?entityID=${id}&entityType=transportation`
      );
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-4">Transportation Images</h1>
