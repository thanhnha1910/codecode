...
const TourImages = () => {
  const { id } = useParams(); // Extract tour ID from URL
  ...
  const apiUrl = `http://localhost:5128/api/images?entityType=tour&entityID=${id}`;
  ...
  const handleAddImages = async (e) => {
    ...
    try {
      const response = await axios.post(
        `http://localhost:5128/api/images/multiple?entityType=tour&entityID=${id}`,
        formData,
        {
          headers: {
  ...
  const handleDeleteImage = async (imageId) => {
    ...
    try {
      ...
      const response = await axios.delete(
        `http://localhost:5128/api/images/${imageId}?entityID=${id}&entityType=tour`
      );
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-4">Tour Images</h1>
