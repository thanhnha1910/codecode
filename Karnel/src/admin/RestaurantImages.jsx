...
const RestaurantImages = () => {
  ...
  const apiUrl = `http://localhost:5128/api/images?entityType=restaurant&entityID=${id}`;
  ...
  const handleAddImages = async (e) => {
    ...
    try {
      const response = await axios.post(
        `http://localhost:5128/api/images/multiple?entityType=restaurant&entityID=${id}`,
        formData,
        {
          headers: {
  ...
  const handleDeleteImage = async (imageId) => {
    ...
    try {
      ...
      const response = await axios.delete(
        `http//localhost:7271/api/images/${imageId}?entityID=${id}&entityType=restaurant`
      );
