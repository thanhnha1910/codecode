...
const CitiesImages = () => {
  const { id } = useParams(); // Extract cities ID from URL
  ...
  const apiUrl = `http://localhost:5128/api/images?entityType=cities&entityID=${id}`;
  console.log("Cities ID:", id);
  ...
  const handleAddImages = async (e) => {
    ...
    try {
      const response = await axios.post(
        `http://localhost:5128/api/images/multiple?entityType=cities&entityID=${id}`,
        formData,
        {
          headers: {
  ...
  const handleDeleteImage = async (imageId) => {
    ...
    try {
      ...
      const response = await axios.delete(
        `http://localhost:5128/api/images/${imageId}?entityID=${id}&entityType=cities`
      );
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-4">Cities Images</h1>
